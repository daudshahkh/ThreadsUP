...
export default function ProductPage() {
  ...
  // Smart Modal Selection Handler
  const handleModalSizeSelect = (size: string) => {
    setSelectedSize(size);
    setIsSelectSizeModalOpen(false);

    // Automatically complete the action they were trying to do
  ...
  if (!product) {
    return <div className="min-h-screen bg-[#0F2B24] text-[#C5A46D] flex items-center justify-center font-serif text-2xl">Product not found.</div>;
  }
      ...
      <AnimatePresence>
        {isSelectSizeModalOpen && (
          <motion.div 
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-[#0B0B0B]/80 backdrop-blur-md"
            onClick={() => setIsSelectSizeModalOpen(false)}
          >
            <motion.div 
              ...
              className="bg-[#0F2B24] border border-dashed border-[#C5A46D] p-8 max-w-sm w-full shadow-2xl relative flex flex-col items-center"
            >
              <button 
                onClick={() => setIsSelectSizeModalOpen(false)}
              ...
              <p className="text-xs text-[#F5E7C1]/60 tracking-widest uppercase mb-6 text-center">
              ...
              <div className="flex gap-3 flex-wrap justify-center w-full">
                {product.sizes.map((size) => (
                  <button
                    ...
                    className="w-14 h-14 flex items-center justify-center border border-dashed border-[#C5A46D]/50 text-[#C5A46D] hover:border-[#C5A46D] hover:bg-[#C5A46D] hover:text-[#0F2B24] transition-all duration-300 font-serif text-xl"
                  >
                    {size}
                  </button>
                ))}
      ...
      <main className="bg-[#0F2B24] text-[#F5E7C1] min-h-screen font-sans selection:bg-[#C5A46D] selection:text-[#0F2B24] relative overflow-hidden flex flex-col">
        ...
        <nav className="relative z-20 p-6 md:px-10 flex justify-between items-center border-b border-dashed border-[#C5A46D]/30 bg-[#0B0B0B]/50 backdrop-blur-md">
          ...
          <div className="hidden lg:flex items-center gap-12 absolute left-1/2 transform -translate-x-1/2">
            <a href="/#collection" className="text-[#F5E7C1]/70 hover:text-[#C5A46D] tracking-widest text-[10px] uppercase transition-colors border-b border-dashed border-transparent hover:border-[#C5A46D] pb-1">
              Collection
            </a>
          ...
          <div className="max-w-4xl w-full grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-14 items-start">
            ...
            <div className="flex flex-col gap-4 w-full mx-auto md:ml-auto" style={{ maxWidth: '360px' }}>
              <motion.div 
                ...
                className="w-full aspect-[4/5] bg-[#0B0B0B] border border-dashed border-[#C5A46D] p-1.5 shadow-xl"
              >
                <img 
              ...
              <div className="flex gap-3 overflow-x-auto pb-1 scrollbar-hide">
                {product.images.map((img, index) => (
                  <button 
                    ...
                    className={`w-16 h-20 flex-shrink-0 relative p-1 transition-all duration-300 ${
                      activeImageIndex === index 
                        ? "border border-dashed border-[#C5A46D] opacity-100 shadow-[0_0_10px_rgba(197,164,109,0.3)]" 
                        : "border border-transparent opacity-50 hover:opacity-90"
                    }`}
                  >
                    <img src={img} alt={`Thumbnail ${index + 1}`} className="w-full h-full object-cover bg-[#0B0B0B]" />
                  </button>
                ))}
              </div>
            </div>
            ...
            <div className="flex flex-col w-full mx-auto md:mr-auto" style={{ maxWidth: '400px' }}>
              <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }}>
                ...
                <p className="text-sm text-[#F5E7C1]/80 font-light leading-relaxed mb-6">
                  {product.description}
                </p>

                <ul className="mb-6 space-y-2">
                  {product.details.map((detail, idx) => (
                    <li key={idx} className="flex items-center text-xs font-light text-[#C5A46D] tracking-wide">
                ...
                <div className="mb-8">
                  <div className="flex justify-between items-center mb-3">
                    ...
                    <button 
                      ...
                      className="text-[10px] text-[#C5A46D] uppercase tracking-widest hover:underline decoration-dashed underline-offset-4"
                    >
                      Size Guide
                    </button>
                  </div>
                  <div className="flex gap-2 flex-wrap">
                    {product.sizes.map((size) => (
                      <button
                        ...
                        className={`w-10 h-10 flex items-center justify-center border border-dashed transition-all duration-300 font-serif text-sm ${
                          selectedSize === size 
                          ? "border-[#C5A46D] bg-[#C5A46D] text-[#0F2B24]" 
                          : "border-[#C5A46D]/50 text-[#C5A46D] hover:border-[#C5A46D] hover:bg-[#153A31]"
                        }`}
                      >
                        {size}
                      </button>
                ...
                <div className="flex gap-3">
                  <button 
                    ...
                    className={`flex-1 py-3 border border-dashed border-[#C5A46D] transition-colors duration-300 tracking-widest text-[10px] uppercase shadow-[0_4px_14px_rgba(0,0,0,0.3)] ${
                      buttonText === "Added ✦"
                      ? "bg-[#C5A46D] text-[#0F2B24]"
                      : "bg-[#0B0B0B] text-[#C5A46D] hover:bg-[#C5A46D] hover:text-[#0F2B24]"
                    }`}
                  >
                    {buttonText}
                  </button>
                  ...
                  <button 
                    ...
                    className="flex-1 py-3 border border-dashed border-[#C5A46D] bg-[#0F2B24] text-[#C5A46D] hover:bg-[#C5A46D] hover:text-[#0F2B24] transition-colors duration-300 tracking-widest text-[10px] uppercase shadow-[0_4px_14px_rgba(0,0,0,0.3)]"
                  >
                    Buy Now
                  </button>
                </div>
